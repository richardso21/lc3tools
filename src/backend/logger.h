...
class Logger {
 ...
 public:
  Logger(IPrinter& printer, uint32_t print_level)
      : printer(printer), print_level(print_level) {}

  lc3::utils::IPrinter& getPrinter(void) const { return printer; }

  template <typename... Args>
  void printf(PrintType level, bool bold, std::string const& format,
              Args... args) const;
  void newline(PrintType level = PrintType::P_ERROR) const {
    if (print_level > static_cast<uint32_t>(level)) {
      printer.newline();
    }
  }
  void print(std::string const& str) {
    if (print_level > static_cast<uint32_t>(PrintType::P_NONE)) {
      printer.print(str);
    }
  }
...
class AssemblerLogger : public Logger {
 public:
  ...
  AssemblerLogger(IPrinter& printer, uint32_t print_level)
      : AssemblerLogger(printer, print_level, "") {}
  AssemblerLogger(IPrinter& printer, uint32_t print_level,
  ...
  void asmPrintf(PrintType level, lc3::core::asmbl::Statement const& statement,
                 std::string const& format, Args... args) const;
  template <typename... Args>
  void asmPrintf(PrintType level, uint32_t row_num, uint32_t col_num,
                 uint32_t len, std::string const& line,
                 std::string const& format, Args... args) const;
...
template <typename... Args>
void lc3::utils::Logger::printf(lc3::utils::PrintType type, bool bold,
                                std::string const& format, Args... args) const {
...
template <typename... Args>
void lc3::utils::AssemblerLogger::asmPrintf(
    lc3::utils::PrintType level, lc3::core::asmbl::Statement const& statement,
    lc3::core::asmbl::StatementPiece const& piece, std::string const& format,
    Args... args) const {
  asmPrintf(level, statement.row, piece.col, piece.len, statement.line, format,
            args...);
}
...
void lc3::utils::AssemblerLogger::asmPrintf(
    lc3::utils::PrintType level, lc3::core::asmbl::Statement const& statement,
    std::string const& format, Args... args) const {
  asmPrintf(level, statement.row, 0, (uint32_t)statement.line.size(),
            statement.line, format, args...);
}

template <typename... Args>
void lc3::utils::AssemblerLogger::asmPrintf(lc3::utils::PrintType level,
                                            uint32_t row_num, uint32_t col_num,
                                            uint32_t len,
                                            std::string const& line,
                                            std::string const& format,
                                            Args... args) const {
  if (static_cast<uint32_t>(level) > print_level) {
    return;
  }

  printer.setColor(lc3::utils::PrintColor::BOLD);
  printer.print(lc3::utils::ssprintf("%s:%d:%d: ", filename.c_str(),
                                     row_num + 1, col_num + 1));
