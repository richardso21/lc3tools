...
lc3::core::Assembler::assemble(std::istream& buffer) {
  ...
  logger.printf(PrintType::P_EXTRA, true,
                "===== begin identifying tokens =====");
  ...
  if (!success) {
    fail_pass = 1;
    goto success_handler;  // we don't want to continue assembling the statement
...
success_handler:
  if (!success) {
    if (fail_pass == 0) {
      ...
      logger.printf(PrintType::P_ERROR, true, "assembly failed (pass %d)",
                    fail_pass);
    }
...
std::pair<bool, std::vector<lc3::core::asmbl::Statement>>
lc3::core::Assembler::buildStatements(std::istream& buffer) {
  using namespace asmbl;
  using namespace lc3::utils;
  ...
  while (!tokenizer.isDone()) {
    ...
    while (!(tokenizer >> cur_token) && cur_token.type != Token::Type::EOL) {
      ...
      logger.printf(PrintType::P_EXTRA, true, " (token) %s",
                    token_str.str().c_str());
#endif
    }
...
std::pair<bool, lc3::core::asmbl::Statement>
lc3::core::Assembler::buildStatement(
  ...
  // A lot of special case logic here to identify tokens as labels,
  // instructions, pseudo-ops, etc. Note: This DOES NOT check for valid
  // statements, it just identifies tokens with what they should be
  //       based on the structure of the statement.
  // Note: There is some redundancy in the code below (not too much), but it was
  // written this way so that it's
  //       easier to follow the flowchart.
  if (tokens.size() > 0) {
    ret.line = tokens[0].line;
    ret.line.erase(
        0, ret.line.find_first_not_of(" \t"));  // trim leading whitespace
    ret.line.erase(ret.line.find_last_not_of(" \t") +
    ...
    if (tokens[0].type == Token::Type::STRING) {
      // If the first token is a string, it could be a label, instruction, or
      // pseudo-op.
      if (encoder.isStringPseudo(tokens[0].str)) {
        ...
        // If the token is not a pseudo-op, it could be either a label or an
        // instruction.
        if (encoder.isStringInstructionName(tokens[0].str)) {
          // The token has been identified to match a valid instruction string,
          // but don't be too hasty in marking it as an instruction yet.
          if (tokens.size() > 1 && tokens[1].type == Token::Type::STRING) {
            if (encoder.isStringPseudo(tokens[1].str)) {
              // If there's a following token that's a pseudo-op, the user
              // probably accidentally used an instruction name as a label, so
              // mark it as such.
              ret.label =
                  StatementPiece{tokens[0], StatementPiece::Type::LABEL};
              ret.base =
                  StatementPiece{tokens[1], StatementPiece::Type::PSEUDO};
              operand_start_idx = 2;
            } else {
              // In most cases, the following token doesn't make a difference
              // and the token really is an instruction.
          ...
          // The first token wasn't identified as an instruction, so it could
          // either be a label or a typo-ed instruction.
          if (tokens.size() > 1) {
            if (tokens[1].type == Token::Type::STRING) {
              if (encoder.isStringPseudo(tokens[1].str)) {
                // If the following token is a pseudo-op, assume the user meant
                // to type a label.
                ret.label =
                    StatementPiece{tokens[0], StatementPiece::Type::LABEL};
                ret.base =
                ...
                // If the following token is a register, assume the user meant
                // to type an instruction
                ret.base =
                    StatementPiece{tokens[0], StatementPiece::Type::INST};
                operand_start_idx = 1;
              } else {
                // If the following token is a string that was not identified as
                // a pseudo-op or register, then treat it as `LABEL inst' if the
                // first operand is a valid instruction. Otherwise
                // conservatively mark as an instruction.
                if (encoder.isStringInstructionName(tokens[1].str)) {
                  ret.label =
                      StatementPiece{tokens[0], StatementPiece::Type::LABEL};
                  ret.base =
                      StatementPiece{tokens[1], StatementPiece::Type::INST};
                  operand_start_idx = 2;
                } else {
                  ret.label =
                      StatementPiece{tokens[0], StatementPiece::Type::INST};
              ...
              // If the following token is a number, assume the user meant to
              // type an instruction
              ret.base = StatementPiece{tokens[0], StatementPiece::Type::INST};
              operand_start_idx = 1;
            }
          } else {
            // If there are no more tokens on the line, just assume the user
            // typed in a label rather than a mis-typed instruction.
    ...
    for (uint32_t i = operand_start_idx; i < tokens.size(); i += 1) {
      if (tokens[i].type == Token::Type::STRING) {
        if (encoder.isStringValidReg(tokens[i].str)) {
          ...
          if (i != tokens.size() - 1) {
            // any register not the last operand should still have a comma from
            // the tokenizer
            if (regToken.str.back() != ',') {
              // signal an error if comma is not present
              logger.asmPrintf(
                  PrintType::P_ERROR, ret,
                  "bad statement format (missing/too many commas?)");
              success = false;
              break;
            }
            // remove the comma
...
void lc3::core::Assembler::setStatementPCField(
    std::vector<lc3::core::asmbl::Statement>& statements) {
  using namespace asmbl;
  using namespace lc3::utils;
  ...
  // Iterate over the statements, setting the current PC every time a new .orig
  // is found.
  while (cur_idx < statements.size()) {
    ...
    if (encoder.isPseudo(statement)) {
      if (encoder.isValidPseudoOrig(statement)) {
        if (found_orig) {
          if (!enable_liberal_asm) {
            if (!previous_region_ended) {
              // If found_orig is set, meaning we've seen at least one valid
              // .orig, and previous_region_ended is not set, meaning we haven't
              // seen a .end yet, then the previous .orig was not ended
              // properly.
              logger.asmPrintf(
                  PrintType::P_ERROR, statement,
                  "new .orig found, but previous region did not have .end");
              logger.newline();
              throw utils::exception(
                  "new .orig fund, but previous region did not have .end");
            }
          }
        ...
        statement.pc = 0;
        ++cur_idx;
        logger.printf(PrintType::P_EXTRA, true, "setting current PC to 0x%0.4x",
                      cur_pc);
        if ((cur_pc & 0xffff) != cur_pc) {
          if (enable_liberal_asm) {
            logger.printf(PrintType::P_WARNING, true,
                          "truncating .orig from 0x%0.4x to 0x%0.4x", cur_pc,
                          cur_pc & 0xffff);
            logger.newline(PrintType::P_WARNING);
          } else {
            logger.printf(PrintType::P_ERROR, true,
                          ".orig 0x%0.4x is out of range", cur_pc);
            logger.newline();
        ...
        // If we see a .end, make sure we've seen at least one .orig already
        // (indicated by found_orig being set).
        if (!found_orig) {
    ...
    if (statement.label && !statement.base) {
      // If the line is only a label, give it the same PC as the line it is
      // pointing to.
      if (!found_orig) {
        statement.valid = false;
      }
    ...
    if (found_orig) {
      if (cur_pc >= MMIO_START) {
        ...
        logger.asmPrintf(PrintType::P_ERROR, statement,
                         "cannot write code into memory-mapped I/O region");
        logger.newline();
        throw utils::exception(
            "cannot write code into memory-mapped I/O region");
      }
      ...
      if (previous_region_ended) {
        // If found_orig and previous_region_ended are both set, that means we
        // have not set previous_region_ended to false yet, which happens when
        // we find a new .orig. In other words, this means there is a line
        // between a .end and a .orig that should be ignored.
        statement.valid = false;
        ++cur_idx;
        continue;
      }
      ...
      statement.pc = cur_pc;
      ++cur_pc;
      logger.printf(PrintType::P_EXTRA, true, "0x%0.4x : \'%s\'", statement.pc,
                    statement.line.c_str());
    } else {
      // If we make it here and haven't found a .orig yet, then there are
      // extraneous lines at the beginning of the file.
      statement.valid = false;
    }
  ...
  for (Statement const& statement : statements) {
    if (statement.label) {
      if (statement.label->type == StatementPiece::Type::NUM) {
        ...
        if (!statement.base) {
          if (statement.operands.size() > 0) {
            for (StatementPiece const& operand : statement.operands) {
              logger.asmPrintf(PrintType::P_ERROR, statement, operand,
                               "illegal operand to a label");
              logger.newline();
            }
            success = false;
            continue;
          } else {
            // Uncommenting following lines will disallow labels on their own
            // line.
            /*
             *if(! enable_liberal_asm) {
             *    logger.asmPrintf(PrintType::P_ERROR, statement,
             **statement.label, "cannot have label on its own line");
             *    logger.newline();
             *    success = false;
             *    continue;
             *}
        ...
        if (search != symbols.end()) {
          ...
          if (enable_liberal_asm) {
            logger.asmPrintf(PrintType::P_WARNING, statement, *statement.label,
                             "redefining label \'%s\' from 0x%0.4x to 0x%0.4x",
                             statement.label->str.c_str(), old_val,
                             statement.pc);
            logger.newline(PrintType::P_WARNING);
          } else {
            logger.asmPrintf(
                PrintType::P_ERROR, statement, *statement.label,
                "attempting to redefine label \'%s\' from 0x%0.4x to 0x%0.4x",
                statement.label->str.c_str(), old_val, statement.pc);
            logger.newline();
            success = false;
            continue;
          }
        ...
        if (!enable_liberal_asm) {
          if ('0' <= statement.label->str[0] &&
              statement.label->str[0] <= '9') {
            logger.asmPrintf(PrintType::P_ERROR, statement, *statement.label,
        ...
        logger.printf(PrintType::P_EXTRA, true,
                      "adding label \'%s\' := 0x%0.4x",
                      statement.label->str.c_str(), statement.pc);
      }
    }
  }
...
std::pair<bool, std::vector<lc3::core::MemLocation>>
lc3::core::Assembler::buildMachineCode(
    std::vector<lc3::core::asmbl::Statement> const& statements,
  ...
  for (Statement const& statement : statements) {
    if (!statement.valid) {
      if (enable_liberal_asm) {
        logger.asmPrintf(
            PrintType::P_WARNING, statement,
            "ignoring statement whose address cannot be determined");
        logger.newline(PrintType::P_WARNING);
      } else {
        logger.asmPrintf(PrintType::P_ERROR, statement,
    ...
    if (statement.base) {
      ...
      // we'll now provide the GUI the symbol table to display labels in a
      // separate column
      if (statement.label) {
      ...
      if (encoder.isPseudo(statement)) {
        ...
        if (valid) {
          if (encoder.isValidPseudoOrig(statement)) {
            ...
            if (statement.pc + size > 0xFFFF) {
              logger.asmPrintf(PrintType::P_ERROR, statement,
                               "block size overflows memory");
              logger.newline();
              success = false;
            }
            std::random_device rd;
            std::uniform_int_distribution<> distr(0, 0xFFFF);
            // .blkw should technically skip memory locations, but we'll fill
            // with random data to mock that
            for (uint32_t i = 0; i < size; i += 1) {
              ret.emplace_back(distr(rd), line, false);
            }
            msg << utils::ssprintf("mem[0x%0.4x:0x%04x] skipped for .blkw",
                                   statement.pc, statement.pc + size - 1);
          } else if (encoder.isValidPseudoString(statement)) {
            ...
            msg << utils::ssprintf("mem[0x%0.4x:0x%04x] = \'%s\\0\'",
                                   statement.pc, statement.pc + value.size(),
                                   value.c_str());
          } else if (encoder.isValidPseudoEnd(statement)) {
            msg << "(end)";
          } else {
#ifdef _ENABLE_DEBUG
            // This should never happen because we already validated the
            // pseudo-op.
            assert(false);
#endif
          }
          ...
          if (valid) {
            ...
            logger.printf(PrintType::P_EXTRA, true, "%s not assembled",
                          msg.str().c_str());
          }
        }
        success &= valid;
        ...
        logger.printf(PrintType::P_EXTRA, true, "%s", msg.str().c_str());
        bool valid = false;
        optional<PIInstruction> candidate =
            encoder.validateInstruction(statement);
        if (candidate) {
          optional<uint32_t> value =
              encoder.encodeInstruction(statement, symbols, *candidate);
          if (value) {
            ret.emplace_back(*value, line, false);
            msg << utils::ssprintf("0x%0.4x", *value);
            valid = true;
...
#ifdef _ENABLE_DEBUG
        // buildStatement should never assign the base field anything other than
        // INST or PSEUDO.
        assert(false);
#endif
      }
