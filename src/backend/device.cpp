...
std::vector<uint16_t> RWReg::getAddrMap(void) const { return {data_addr}; }

KeyboardDevice::KeyboardDevice(lc3::utils::IInputter& inputter)
    : inputter(inputter) {
  status.setValue(0x0000);
  data.setValue(0x0000);
}

void KeyboardDevice::startup(void) { inputter.beginInput(); }

void KeyboardDevice::shutdown(void) { inputter.endInput(); }

std::pair<uint16_t, PIMicroOp> KeyboardDevice::read(uint16_t addr) {
  if (addr == KBSR) {
    PIMicroOp callback =
        std::make_shared<CallbackMicroOp>(CallbackType::INPUT_POLL);
    ...
    if (utils::getBit(status_value, 15) == 1) {
      ...
      PIMicroOp toggle_status =
          std::make_shared<MemWriteImmMicroOp>(8, status_value & 0x7FFF);
      PIMicroOp pop_from_buffer =
          std::make_shared<GenericPopMicroOp<std::queue<KeyInfo>>>(key_buffer,
                                                                   "kbbuf");
...
PIMicroOp KeyboardDevice::tick(void) {
  ...
  if (!key_buffer.empty()) {
    ...
    if (!key_buffer.front().triggered_interrupt &&
        (status.getValue() & 0x4000) == 0x4000) {
      key_buffer.front().triggered_interrupt = true;
      return std::make_shared<PushInterruptTypeMicroOp>(
