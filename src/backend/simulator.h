...
class Simulator {
 public:
  ...
  Simulator(lc3::utils::IPrinter &printer, lc3::utils::IInputter &inputter,
 ...
 private:
  std::priority_queue<PIEvent, std::vector<PIEvent>, std::greater<PIEvent>>
      events;
  uint64_t time;
  ...
  static void callbackDispatcher(Simulator *sim, CallbackType type,
                                 MachineState &state);
};
};  // namespace core
};  // namespace lc3
