...
PIMicroOp ADDRegInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp compute = std::make_shared<RegAddRegMicroOp>(
      dst_id, getOperand(2)->getValue(), getOperand(4)->getValue());
...
PIMicroOp ADDImmInstruction::buildMicroOps(MachineState const& state) const {
  ...
  uint16_t dst_id = getOperand(1)->getValue();
  PIMicroOp compute = std::make_shared<RegAddImmMicroOp>(
      dst_id, getOperand(2)->getValue(),
      lc3::utils::sextTo16(getOperand(4)->getValue(),
                           getOperand(4)->getWidth()));
  PIMicroOp set_cc = std::make_shared<CCUpdateRegMicroOp>(dst_id);
...
PIMicroOp ANDRegInstruction::buildMicroOps(MachineState const& state) const {
  ...
  uint16_t dst_id = getOperand(1)->getValue();
  PIMicroOp compute = std::make_shared<RegAndRegMicroOp>(
      dst_id, getOperand(2)->getValue(), getOperand(4)->getValue());
  PIMicroOp set_cc = std::make_shared<CCUpdateRegMicroOp>(dst_id);
...
PIMicroOp ANDImmInstruction::buildMicroOps(MachineState const& state) const {
  ...
  uint16_t dst_id = getOperand(1)->getValue();
  PIMicroOp compute = std::make_shared<RegAndImmMicroOp>(
      dst_id, getOperand(2)->getValue(),
      lc3::utils::sextTo16(getOperand(4)->getValue(),
                           getOperand(4)->getWidth()));
  PIMicroOp set_cc = std::make_shared<CCUpdateRegMicroOp>(dst_id);
...
PIMicroOp BRInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp jump = std::make_shared<PCAddImmMicroOp>(lc3::utils::sextTo16(
      getOperand(2)->getValue(), getOperand(2)->getWidth()));

  return std::make_shared<BranchMicroOp>(
      [this](MachineState const& state) {
        return (getOperand(1)->getValue() &
                lc3::utils::getBits(state.readPSR(), 2, 0)) != 0;
      },
...
PIMicroOp JMPInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp callback =
      std::make_shared<CallbackMicroOp>(CallbackType::SUB_EXIT);
  PIMicroOp func_trace = std::make_shared<PopFuncTypeMicroOp>();

  jump->insert(std::make_shared<BranchMicroOp>(
      [this](MachineState const& state) {
        return (getOperand(2)->getValue() == 7 &&
                state.peekFuncTraceType() == FuncType::SUBROUTINE);
      },
...
PIMicroOp JSRInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp jump = std::make_shared<PCAddImmMicroOp>(lc3::utils::sextTo16(
      getOperand(2)->getValue(), getOperand(2)->getWidth()));
  PIMicroOp callback =
      std::make_shared<CallbackMicroOp>(CallbackType::SUB_ENTER);
  PIMicroOp func_trace =
      std::make_shared<PushFuncTypeMicroOp>(FuncType::SUBROUTINE);
...
PIMicroOp JSRRInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp jump = std::make_shared<PCWriteRegMicroOp>(
      getOperand(3)->getValue());                                // PC=BaseR
...
PIMicroOp LDInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp compute_addr = std::make_shared<RegAddImmMicroOp>(
      8, 8,
      lc3::utils::sextTo16(getOperand(2)->getValue(),
...
PIMicroOp LDRInstruction::buildMicroOps(MachineState const& state) const {
  ...
  PIMicroOp compute_addr = std::make_shared<RegAddImmMicroOp>(
      8, 8,
      lc3::utils::sextTo16(getOperand(3)->getValue(),
                           getOperand(3)->getWidth()));
  PIMicroOp load = std::make_shared<MemReadMicroOp>(dst_id, 8);
  PIMicroOp set_cc = std::make_shared<CCUpdateRegMicroOp>(dst_id);
...
PIMicroOp NOTInstruction::buildMicroOps(MachineState const& state) const {
  ...
  uint16_t dst_id = getOperand(1)->getValue();
  PIMicroOp compute =
      std::make_shared<RegNotMicroOp>(dst_id, getOperand(2)->getValue());
  PIMicroOp set_cc = std::make_shared<CCUpdateRegMicroOp>(dst_id);
...
PIMicroOp RTIInstruction::buildMicroOps(MachineState const& state) const {
  ...
  std::pair<PIMicroOp, PIMicroOp> handle_exception_chain = buildSystemModeEnter(
      INTEX_TABLE_START, 0x0, lc3::utils::getBits(state.readPSR(), 10, 8));
  PIMicroOp ex_callback =
      std::make_shared<CallbackMicroOp>(CallbackType::EX_ENTER);
  PIMicroOp ex_func_trace =
  ...
  switch (state.peekFuncTraceType()) {
    case FuncType::TRAP:
      callback = std::make_shared<CallbackMicroOp>(CallbackType::SUB_EXIT);
      break;
    case FuncType::INTERRUPT:
      callback = std::make_shared<CallbackMicroOp>(CallbackType::INT_EXIT);
      break;
    case FuncType::EXCEPTION:
      callback = std::make_shared<CallbackMicroOp>(CallbackType::EX_EXIT);
      break;
    default:
      break;
  }
  ...
  PIMicroOp start = std::make_shared<BranchMicroOp>(
      [](MachineState const& state) {
  ...
  dec_sp2->insert(std::make_shared<BranchMicroOp>(
      [](MachineState const& state) {
        return lc3::utils::getBit(state.readPSR(), 15) == 0;
      },
...
PIMicroOp TRAPInstruction::buildMicroOps(MachineState const& state) const {
  std::pair<PIMicroOp, PIMicroOp> handle_trap_chain = buildSystemModeEnter(
      TRAP_TABLE_START, static_cast<uint8_t>(getOperand(2)->getValue()),
      (state.readPSR() & 0x0700) >> 8);
  PIMicroOp callback =
      std::make_shared<CallbackMicroOp>(CallbackType::SUB_ENTER);
...
bool lc3::core::isAccessViolation(uint16_t addr, MachineState const& state) {
  if (state.getIgnorePrivilege()) {
    ...
    return lc3::utils::getBit(state.readPSR(), 15) == 1 &&
           (addr <= SYSTEM_END || addr >= MMIO_START);
  }
}

std::pair<PIMicroOp, PIMicroOp> lc3::core::buildSystemModeEnter(
    uint16_t table_start, uint8_t vec, uint8_t priority) {
  ...
  PIMicroOp set_priority =
      std::make_shared<RegAddImmMicroOp>(10, 10, (priority & 0x7) << 8);
  ...
  PIMicroOp write_table_start =
      std::make_shared<RegWriteImmMicroOp>(11, table_start);
  ...
  write_priority->insert(std::make_shared<BranchMicroOp>(
      [](MachineState const& state) {
        return lc3::utils::getBit(state.readPSR(), 15) == 1;
      },
