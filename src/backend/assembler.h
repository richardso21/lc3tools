...
class Assembler {
 public:
  Assembler(lc3::utils::IPrinter &printer, uint32_t print_level,
            bool enable_liberal_asm)
      : logger(printer, print_level),
  ...
  std::pair<std::shared_ptr<std::stringstream>, SymbolTable> assemble(
      std::istream &buffer);
 ...
 private:
  ...
  std::pair<bool, std::vector<asmbl::Statement>> buildStatements(
      std::istream &buffer);
  std::pair<bool, asmbl::Statement> buildStatement(
      std::vector<asmbl::Token> const &tokens);
  void setStatementPCField(std::vector<asmbl::Statement> &statements);
  std::pair<bool, SymbolTable> buildSymbolTable(
      std::vector<asmbl::Statement> const &statements);
  std::pair<bool, std::vector<MemLocation>> buildMachineCode(
      std::vector<asmbl::Statement> const &statements,
