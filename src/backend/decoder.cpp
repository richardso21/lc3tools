...
lc3::optional<lc3::core::PIInstruction> Decoder::decode(uint16_t value) const {
  ...
  if (search != instructions_by_opcode.end()) {
    ...
    for (PIInstruction inst : search->second) {
      ...
      valid = true;
      // Scan over all fixed operands in instruction to determine if there's a
      // match.
      for (PIOperand const op : inst->getOperands()) {
        if (op->getType() == IOperand::Type::FIXED) {
          if (lc3::utils::getBits(value, bit_check_pos,
                                  bit_check_pos - op->getWidth() + 1) !=
              op->getValue()) {
            valid = false;
            break;
          }
      ...
      if (valid) {
        ...
        for (PIOperand op : inst->getOperands()) {
          if (op->getType() != IOperand::Type::FIXED) {
            op->setValue(lc3::utils::getBits(value, bit_pos,
                                             bit_pos - op->getWidth() + 1));
          }
          bit_pos -= op->getWidth();
        }
        return inst;
