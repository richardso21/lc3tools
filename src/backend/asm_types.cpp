...
lc3::optional<uint32_t> lc3::core::asmbl::getNum(
    Statement const &statement, StatementPiece const &piece, uint32_t width,
  ...
  if (sext) {
    ...
    if (signed_value < -(1 << (width - 1)) ||
        signed_value > ((1 << (width - 1)) - 1)) {
      if (log_enable) {
        logger.asmPrintf(utils::PrintType::P_ERROR, statement, piece,
                         "cannot encode as %d-bit 2's "
                         "complement number",
                         width);
        logger.newline();
      }
