...
std::ostream &lc3::core::operator<<(std::ostream &out,
                                    lc3::core::MemLocation const &in) {
#ifdef _ENABLE_DEBUG_ASM
  out << lc3::utils::ssprintf("0x%0.4x", in.value) << " " << in.orig << " "
      << in.line << "\n";
  return out;
#else
  // TODO: this is extrememly unportable, namely because it relies on the
  // endianness not changing encoding (2 bytes), then orig bool (1 byte), then
  // number of characters (4 bytes), then actual line (N bytes, not null
  // terminated)
