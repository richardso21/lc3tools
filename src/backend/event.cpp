...
std::string AtomicInstProcessEvent::toString(MachineState const& state) const {
  return lc3::utils::ssprintf("Processing M[0x%0.4x]:0x%0.4x (%s)",
                              state.readPC(),
                              std::get<0>(state.readMem(state.readPC())),
                              state.getMemLine(state.readPC()).c_str());
}
...
void PowerOnEvent::handleEvent(MachineState& state) { state.writeMCR(0x8000); }
...
void LoadObjFileEvent::handleEvent(MachineState& state) {
  ...
  if (buffer.read(header, expected_header.size())) {
    for (uint32_t i = 0; i < expected_header.size(); i += 1) {
      if (header[i] != expected_header[i]) {
        logger.printf(
            PrintType::P_ERROR, true,
            "invalid header (is this a .obj file?); try re-assembling");
        throw lc3::utils::exception(
            "invalid header (is this a .obj file?); try re-assembling");
      }
    }
    delete[] header;
  ...
  if (buffer.read(version, expected_version.size())) {
    for (uint32_t i = 0; i < expected_version.size(); i += 1) {
      if (version[i] != expected_version[i]) {
        logger.printf(PrintType::P_ERROR, true,
                      "mismatched version numbers; try re-assembling");
        throw lc3::utils::exception(
            "mismatched version numbers; try re-assembling");
      }
    }
    ...
    delete[] version;
    logger.printf(PrintType::P_ERROR, true,
                  "could not read version number; try re-assembling");
    throw lc3::utils::exception(
        "could not read version number; try re-assembling");
  }
  ...
  while (!buffer.eof()) {
    ...
    // enforce that all code should start at user space (0x3000) unless the
    // first .orig is at a higher address
    state.writeResetPC(USER_START);

    if (mem.isOrig()) {
      if (!reset_pc_set) {
        if (mem.getValue() >= USER_START) {
          // if orig is not at user space, then most likely an interrupt or OS
          // is being loaded
      ...
      logger.printf(lc3::utils::PrintType::P_DEBUG, true,
                    "0x%0.4x: %s (0x%0.4x)", fill_pc + offset,
...
void CheckForInterruptEvent::handleEvent(MachineState& state) {
  ...
  if (interrupt != InterruptType::INVALID &&
      (getInterruptPriority(interrupt) >
       lc3::utils::getBits(state.readPSR(), 10, 8))) {
    std::pair<PIMicroOp, PIMicroOp> handle_interrupt_chain =
        buildSystemModeEnter(INTEX_TABLE_START, getInterruptVector(interrupt),
                             getInterruptPriority(interrupt));
    PIMicroOp dequeue_interrupt = std::make_shared<PopInterruptTypeMicroOp>();
    PIMicroOp callback =
        std::make_shared<CallbackMicroOp>(CallbackType::INT_ENTER);
    PIMicroOp func_trace =
        std::make_shared<PushFuncTypeMicroOp>(FuncType::INTERRUPT);
...
std::string CheckForInterruptEvent::toString(MachineState const& state) const {
  ...
  if (interrupt != InterruptType::INVALID &&
      (getInterruptPriority(interrupt) >
       lc3::utils::getBits(state.readPSR(), 10, 8))) {
    return lc3::utils::ssprintf("Handling %s interrupt",
                                interruptTypeToString(interrupt).c_str());
  } else {
    return "No interrupt of higher priority pending";
  }
}

void CallbackEvent::handleEvent(MachineState& state) { func(type, state); }

std::string CallbackEvent::toString(MachineState const& state) const {
  (void)state;

  return lc3::utils::ssprintf("Triggering %s callback",
