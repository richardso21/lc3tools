...
class RegWriteImmMicroOp : public IMicroOp {
 public:
  RegWriteImmMicroOp(uint16_t reg_id, uint16_t value)
      : IMicroOp(), reg_id(reg_id), value(value) {}
...
class RegWriteRegMicroOp : public IMicroOp {
 public:
  RegWriteRegMicroOp(uint16_t dst_id, uint16_t src_id)
...
class BranchMicroOp : public IMicroOp {
 public:
  ...
  BranchMicroOp(PredFunction pred, std::string msg, PIMicroOp true_next,
                PIMicroOp false_next)
...
class GenericPopMicroOp : public IMicroOp {
 public:
  GenericPopMicroOp(T& data, std::string const& name)
      : IMicroOp(), data(data), name(name) {}
  ...
  virtual std::string toString(MachineState const& state) const override {
    (void)state;
    return lc3::utils::ssprintf("%s <= %s.removeTop()", name.c_str(),
                                name.c_str());
  }
...
std::pair<PIMicroOp, PIMicroOp> buildSystemModeEnter(uint16_t table_start,
                                                     uint8_t vec,
                                                     uint8_t priority);
};  // namespace core
};  // namespace lc3
