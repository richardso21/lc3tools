...
class IOperand {
 public:
  enum class Type { FIXED = 0, NUM, LABEL, REG, INVALID };

  IOperand(Type type, std::string const& type_str, uint32_t width);
  virtual ~IOperand(void) = default;

  virtual optional<uint32_t> encode(asmbl::Statement const& statement,
                                    asmbl::StatementPiece const& piece,
                                    SymbolTable const& regs,
...
class FixedOperand : public IOperand {
 public:
  ...
  virtual optional<uint32_t> encode(asmbl::Statement const& statement,
                                    ...
                                    SymbolTable const& regs,
                                    SymbolTable const& symbols,
                                    utils::AssemblerLogger& logger) override;
};
