...
struct TestCase {
  ...
  TestCase(char const *name,
           std::function<void(lc3::sim &, StringInputter &)> test_func,
           uint32_t points, bool randomize) {
    this->name = name;
...
#define VERIFY(check) VERIFY_NAMED(#check, check)
...
#define VERIFY_OUTPUT(check) VERIFY_OUTPUT_NAMED(#check, check)
...
#define VERIFY_OUTPUT_HAD(check) VERIFY_OUTPUT_HAD_NAMED(#check, check)
