...
void verify(Tester& tester, bool success, std::string const& expected,
            std::string const& label, bool not_present, double points) {
  if (!success) {
...
void ExampleTest(lc3::sim& sim, Tester& tester, double total_points) {
  ...
  verify(tester, success, "55555", "Contains correct count", false,
         total_points / 2);
  verify(tester, success, "555555", "Does not contain incorrect count", true,
         total_points / 2);
}
...
void PrevASCIITest(lc3::sim& sim, Tester& tester, double total_points) {
  ...
  tester.setInputCharDelay(1000);
  bool success = sim.run();
  verify(tester, success, "@ is not a decimal digit", "Correct behavior", false,
         total_points);
}
...
void testBringup(lc3::sim& sim) { sim.setRunInstLimit(10000); }
