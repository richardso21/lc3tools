...
void verify(Tester& tester, lc3::sim& sim, bool success, uint16_t expected_val,
            uint64_t expected_sub_count, double points) {
  if (!success) {
    tester.error("Error", "Execution hit exception");
    return;
  }

  tester.verify("Correct functionality", sim.readMem(0x4000) == expected_val,
                points * 0.8);
  tester.verify("Correct time complexity", sub_count == expected_sub_count,
                points * 0.2);
}
